...
import com.donniebirb.stealthpixeldungeon.windows.WndError;
import com.donniebirb.stealthpixeldungeon.windows.WndHardNotification;
...
public class WelcomeScene extends PixelScene {
	...
	private static final int LATEST_UPDATE = StealthPixelDungeon.v0_1_0;
	...
	public void create() {
		...
		if (!triedCleaningTemp && FileUtils.cleanTempFiles()){
			add(new WndHardNotification(Icons.get(Icons.WARNING),
				...
				public void hide() {
					...
					StealthPixelDungeon.resetScene();
				}
			});
			return;
		}

		if (StealthPixelDungeon.versionCode == previousVersion && !SPDSettings.intro()) {
		...
		StyledButton okay = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(this, "continue")){
			...
			protected void onClick() {
				...
				if (previousVersion == 0 || SPDSettings.intro()){
					...
					SPDSettings.version(StealthPixelDungeon.versionCode);
		...
		if (previousVersion != 0 && !SPDSettings.intro()){
			StyledButton changes = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(TitleScene.class, "changes")){
				...
				protected void onClick() {
					super.onClick();
					updateVersion(previousVersion);
					StealthPixelDungeon.switchScene(ChangesScene.class);
				}
			};
		...
		} else if (previousVersion <= StealthPixelDungeon.versionCode) {
