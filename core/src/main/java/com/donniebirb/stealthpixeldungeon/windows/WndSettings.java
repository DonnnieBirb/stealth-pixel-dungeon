...
import com.donniebirb.stealthpixeldungeon.ui.CheckBox;
import com.donniebirb.stealthpixeldungeon.ui.GameLog;
import com.donniebirb.stealthpixeldungeon.ui.Icons;
import com.donniebirb.stealthpixeldungeon.ui.OptionSlider;
...
public class WndSettings extends WndTabbed {
	...
	public void hide() {
		...
		StealthPixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
			@Override
			...
			if (DeviceCompat.isAndroid() && PixelScene.maxScreenZoom >= 2) {
				chkSaver = new CheckBox(Messages.get(this, "saver")) {
					...
					protected void onClick() {
						...
						if (checked()) {
							checked(!checked());
							StealthPixelDungeon.scene().add(new WndOptions(Icons.get(Icons.DISPLAY),
		...
		protected void createChildren() {
			...
			if (SPDSettings.interfaceSize() == 0) {
				btnToolbarSettings = new RedButton(Messages.get(this, "toolbar_settings"), 9){
					@Override
					protected void onClick() {
						StealthPixelDungeon.scene().addToFront(new Window(){
		...
		protected void createChildren() {
			...
			if (DeviceCompat.hasHardKeyboard()){
				...
				btnKeyBindings = new RedButton(Messages.get(this, "key_bindings")){
					...
					protected void onClick() {
						super.onClick();
						StealthPixelDungeon.scene().addToFront(new WndKeyBindings(false));
					}
				};
			...
			if (ControllerHandler.isControllerConnected()){
				btnControllerBindings = new RedButton(Messages.get(this, "controller_bindings")){
					...
					protected void onClick() {
						super.onClick();
						StealthPixelDungeon.scene().addToFront(new WndKeyBindings(true));
					}
				};
			...
			if (currLang != Languages.ENGLISH) {
				...
				btnCredits = new RedButton(credText, credText.length() > 9 ? 6 : 9) {
					...
					protected void onClick() {
						...
						StealthPixelDungeon.scene().addToFront(credits);
					}
				};
				add(btnCredits);
			}
